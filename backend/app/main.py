...
# App Setup
# ─────────────────────────────────────────────
app = FastAPI(title="ForeGuard API")
...
def classify_types(regions):
    """Map low-level detector types → high-level forgery categories"""
    types = set()

    for r in regions:
        t = r.get("type", "").lower()

        if any(x in t for x in ["font", "text", "ocr", "background", "numeric"]):
            types.add("TEXT_MANIPULATION")

        elif any(x in t for x in ["image", "ela"]):
...
async def detect_forgery(
    ...
    try:
        ...
        ocr_detector = OCRDetector(lang_list)
        ...
        weighted_conf = sum(
            r.confidence * w for r, w in zip(detection_results, weights)
        )
